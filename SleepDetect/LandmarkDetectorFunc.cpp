...
void UpdateTemplate(const cv::Mat_<uchar> &grayscale_image, CLNF& clnf_model)
{
	...
	/*cv::Mat result = grayscale_image(bounding_box).clone();
	cv::resize(result, result, cv::Size(480, 320));
	clnf_model.face_template = result.clone();*/
	//clnf_model.face_template = grayscale_image(bounding_box).clone();

	//cv::rectangle(grayscale_image, bounding_box.tl(), bounding_box.br(), cv::Scalar(0, 0, 255), 3);

	//cv::imshow("tracking1", clnf_model.face_template);
...
cv::Rect LandmarkDetector::DetectLandmarksInVideo(const cv::Mat_<uchar> &grayscale_image, const cv::Mat_<float> &depth_image, CLNF& clnf_model, FaceModelParameters& params, bool &track_success_return)
{
	...
	if(clnf_model.tracking_initialised)
	{
		...
		//cv::Mat testMat = grayscale_image(rectT);
		
		//// Make sure the box is not out of bounds
		////rectT = bounding_box & cv::Rect(0, 0, grayscale_image.cols, grayscale_image.rows);
		//cv::Mat test = grayscale_image.clone();
		//cv::rectangle(test, rectT, cv::Scalar(0, 0, 255));
		//cv::imshow("track", test);

		bool track_success =  clnf_model.DetectLandmarks(grayscale_image/*testMat*/, depth_image, params);

		track_success_return = track_success;
  ...
  //          clnf_model.face_detector_HAAR.load(params.face_detector_location);
		...
		bool face_detection_success = true;
...
//		else
  ...
  //     if(params.curr_face_detector == FaceModelParameters::HAAR_DETECTOR)
      ...
      /*  bounding_box.x = 100;
        ...
        bounding_box.height = 300;*/
        //cv::rectangle(grayscale_image, bounding_box.tl(),  bounding_box.br(), cv::Scalar(0,0,255), 3);
        
        //cv::imshow("tracking", grayscale_image);
        
		//cv::Rect_<double> bounding_box(40, 40, grayscale_image.size().width-80, grayscale_image.size().height-80);

		cv::Rect_<double> bounding_box(0, 0, grayscale_image.cols, grayscale_image.rows);
		...
		if (face_detection_success)
		{
			...
			cv::Mat testMat;
			if(rectT.x + rectT.width > grayscale_image.cols || rectT.y + rectT.height > grayscale_image.rows || rectT.x < 1 || rectT.y < 1)
				testMat = grayscale_image.clone();
			else
				testMat = grayscale_image(rectT);

			bool landmark_detection_success = clnf_model.DetectLandmarks(grayscale_image/*testMat*/, depth_image, params);


			track_success_return = landmark_detection_success;
...
bool LandmarkDetector::DetectLandmarksInVideo(const cv::Mat_<uchar> &grayscale_image, const cv::Mat_<float> &depth_image, const cv::Rect_<double> bounding_box, CLNF& clnf_model, FaceModelParameters& params)
{
	...
	bool test = false;
	cv:: Rect rectBack = DetectLandmarksInVideo(grayscale_image, depth_image, clnf_model, params, test);
	
	return rectBack.area()>100;
...
{
	bool test = false;
	cv::Rect rectBack = DetectLandmarksInVideo(grayscale_image, cv::Mat_<float>(), clnf_model, params, test);
